...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(original_seq) >= maxlen:
            emb_i = np.random.randint(0, maxlen)
        else:
            emb_i = np.random.randint(maxlen - len(original_seq), maxlen)
        ...
        return {"input_seq": torch.tensor(seq), "sample_seq": torch.tensor(sample_seq), "emb_i": emb_i} 
