...
class LightSAS(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        log_seqs, samples, emb_indices = batch["input_seq"], batch["sample_seq"], batch["emb_i"]
        ...
        #dense all logic to select random embeddings to prop loss through

        '''actual_len = (log_seqs != 0).sum(-1) #for each (user, pos) ignoring padding indices to choose embedding from

        #print(actual_len)
        emb_indices = []
        for i in range(log_seqs.shape[0]):
            emb_indices.append(torch.randint(log_seqs.shape[1] - actual_len[i], log_seqs.shape[1], (1,)).item()) #random emb for each (user, pos)
        
        emb_indices = torch.tensor(emb_indices, device = self.device)'''

        #print("selected indices: ", emb_indices)
        emb_index = emb_indices[:,None,None].expand(log_feats.shape[0], -1, log_feats.shape[-1]) #prep for gather


        selected_feats = torch.gather(log_feats, 1, emb_index) #batch_sizex1xdim #one selected feature per (user, pos)
        ##print("Forward ended")
        ...
        #sample_logits = (sample_embs * selected_feats).sum(dim=-1)
        #or (prob faster below)
        sample_logits = (sample_embs @ selected_feats.transpose(-1,-2)).squeeze(-1)
